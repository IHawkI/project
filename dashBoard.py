...
if vista_general or not vista_filtrada:
    ...
    with col1:
        ...
        sns.histplot(df['Rating'], bins=30, kde=True, ax=ax, edgecolor="black", color="lightgray")
        ...
        en_rango = len(df[(df['Total Price'] >= 550) & (df['Total Price'] <= 800)])
    ...
    with col4:
        fig, ax = plt.subplots(figsize=(5, 4))
        sns.histplot(df['Age'], bins=30, kde=True, color="#3070b8", ax=ax)
